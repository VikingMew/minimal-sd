...
def create_infotext(
    ...
    generation_params = {
        ...
        "Model": (
            ...
            or not shared.sd_model.sd_checkpoint_info.model_name
    ...
    return (
        f"{all_prompts[index]}{negative_prompt_text}\n{generation_params_text}".strip()
    )
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            negative_prompts = p.all_negative_prompts[
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if p.color_corrections is not None and i < len(p.color_corrections):
                    if (
                        opts.save
                        and not p.do_not_save_samples
                        and opts.save_images_before_color_correction
                    ):
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def sample(
        ...
        def save_intermediate(image, index):
            ...
            if (
                not opts.save
                or self.do_not_save_samples
                or not opts.save_images_before_highres_fix
            ):
                return
