...
def apply_optimizations():
    ...
    if cmd_opts.force_enable_xformers or (
        cmd_opts.xformers
        and shared.xformers_available
        and torch.version.cuda
        and (6, 0) <= torch.cuda.get_device_capability(shared.device) <= (9, 0)
    ):
        print("Applying xformers cross attention optimization.")
        ldm.modules.attention.CrossAttention.forward = (
            sd_hijack_optimizations.xformers_attention_forward
        )
        ldm.modules.diffusionmodules.model.AttnBlock.forward = (
            sd_hijack_optimizations.xformers_attnblock_forward
        )
        ...
        ldm.modules.attention.CrossAttention.forward = (
            sd_hijack_optimizations.scaled_dot_product_no_mem_attention_forward
        )
        ldm.modules.diffusionmodules.model.AttnBlock.forward = (
            sd_hijack_optimizations.sdp_no_mem_attnblock_forward
        )
        optimization_method = "sdp-no-mem"
    elif cmd_opts.opt_sdp_attention and can_use_sdp:
        print("Applying scaled dot product cross attention optimization.")
        ldm.modules.attention.CrossAttention.forward = (
            sd_hijack_optimizations.scaled_dot_product_attention_forward
        )
        ldm.modules.diffusionmodules.model.AttnBlock.forward = (
            sd_hijack_optimizations.sdp_attnblock_forward
        )
        optimization_method = "sdp"
    elif cmd_opts.opt_sub_quad_attention:
        print("Applying sub-quadratic cross attention optimization.")
        ldm.modules.attention.CrossAttention.forward = (
            sd_hijack_optimizations.sub_quad_attention_forward
        )
        ldm.modules.diffusionmodules.model.AttnBlock.forward = (
            sd_hijack_optimizations.sub_quad_attnblock_forward
        )
        ...
        cmd_opts.opt_split_attention_invokeai
        or not cmd_opts.opt_split_attention
        and not torch.cuda.is_available()
    ):
        print("Applying cross attention optimization (InvokeAI).")
        ldm.modules.attention.CrossAttention.forward = (
            sd_hijack_optimizations.split_cross_attention_forward_invokeAI
        )
