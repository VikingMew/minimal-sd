...
def save_pic_with_caption(
    image, index, params: PreprocessParams, existing_caption=None
):
    caption = ""
    ...
    if len(caption) > 0:
        with open(
            os.path.join(params.dstdir, f"{basename}.txt"), "w", encoding="utf8"
        ) as file:
            file.write(caption)
...
def multicrop_pic(image: Image, mindim, maxdim, minarea, maxarea, objective, threshold):
    ...
    wh = max(
        ...
        key=lambda wh: (wh[0] * wh[1], -err(*wh))[
            :: 1 if objective == "Maximize area" else -1
        ],
        default=None,
    )
        ...
        if process_focal_crop and img.height != img.width:
            ...
            try:
                dnn_model_path = autocrop.download_and_cache_models(
                    os.path.join(paths.models_path, "opencv")
                )
