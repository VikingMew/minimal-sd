...
class CFGDenoiser(torch.nn.Module):
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, image_cond):
            ...
            if shared.batch_cond_uncond:
                ...
                for batch_offset in range(0, x_out.shape[0], batch_size):
                    ...
                    x_out[a:b] = self.inner_model(
                        x_in[a:b],
                        sigma_in[a:b],
                        cond=make_condition_dict([cond_in[a:b]], image_cond_in[a:b]),
                    )
        else:
            x_out = torch.zeros_like(x_in)
        ...
        denoised_params = CFGDenoisedParams(
            x_out, state.sampling_step, state.sampling_steps
        )
...
class KDiffusionSampler:
    ...
    def initialize(self, p):
        ...
        k_diffusion.sampling.torch = TorchHijack(
            self.sampler_noises if self.sampler_noises is not None else []
        )
