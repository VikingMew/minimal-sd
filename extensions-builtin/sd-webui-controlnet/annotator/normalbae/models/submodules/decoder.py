...
class Decoder(nn.Module):
    ...
    def forward(self, features, gt_norm_mask=None, mode="test"):
        ...
        if mode == "train":
            ...
            out_res8_res4 = F.interpolate(
                out_res8, scale_factor=2, mode="bilinear", align_corners=True
            )
            B, _, H, W = out_res8_res4.shape

            # samples: [B, 1, N, 2]
            point_coords_res4, rows_int, cols_int = sample_points(
                out_res8_res4.detach(),
                gt_norm_mask,
            ...
            feat_res4 = F.grid_sample(
                x_d2, point_coords_res4, mode="bilinear", align_corners=True
            )  # (B, 512, 1, N)
            init_pred = F.grid_sample(
                out_res8, point_coords_res4, mode="bilinear", align_corners=True
            )  # (B, 4, 1, N)
            ...
            samples_pred_res4 = norm_normalize(
                samples_pred_res4
            )  # (B, 4, N) - normalized

            for i in range(B):
                out_res4[i, :, rows_int[i, :], cols_int[i, :]] = samples_pred_res4[
                    i, :, :
                ]
        ...
        if mode == "train":
            ...
            out_res4_res2 = F.interpolate(
                out_res4, scale_factor=2, mode="bilinear", align_corners=True
            )
            B, _, H, W = out_res4_res2.shape

            # samples: [B, 1, N, 2]
            point_coords_res2, rows_int, cols_int = sample_points(
                out_res4_res2.detach(),
                gt_norm_mask,
                sampling_ratio=self.sampling_ratio,
                beta=self.importance_ratio,
            )
            ...
            # grid_sample feature-map
            feat_res2 = F.grid_sample(
                x_d3, point_coords_res2, mode="bilinear", align_corners=True
            )  # (B, 256, 1, N)
            init_pred = F.grid_sample(
                out_res4, point_coords_res2, mode="bilinear", align_corners=True
            )  # (B, 4, 1, N)
            feat_res2 = torch.cat([feat_res2, init_pred], dim=1)  # (B, 256+4, 1, N)

            # prediction (needed to compute loss)
            samples_pred_res2 = self.out_conv_res2(feat_res2[:, :, 0, :])  # (B, 4, N)
            samples_pred_res2 = norm_normalize(
                samples_pred_res2
            )  # (B, 4, N) - normalized

            for i in range(B):
                out_res2[i, :, rows_int[i, :], cols_int[i, :]] = samples_pred_res2[
                    i, :, :
                ]
        ...
        if mode == "train":
            # upsampling ... out_res4: [2, 4, 120, 160] -> out_res4_res2: [2, 4, 240, 320]
            out_res2_res1 = F.interpolate(
                out_res2, scale_factor=2, mode="bilinear", align_corners=True
            )
            B, _, H, W = out_res2_res1.shape

            # samples: [B, 1, N, 2]
            point_coords_res1, rows_int, cols_int = sample_points(
                out_res2_res1.detach(),
                gt_norm_mask,
                sampling_ratio=self.sampling_ratio,
                beta=self.importance_ratio,
            )
            ...
            # grid_sample feature-map
            feat_res1 = F.grid_sample(
                x_d4, point_coords_res1, mode="bilinear", align_corners=True
            )  # (B, 128, 1, N)
            init_pred = F.grid_sample(
                out_res2, point_coords_res1, mode="bilinear", align_corners=True
            )  # (B, 4, 1, N)
            feat_res1 = torch.cat([feat_res1, init_pred], dim=1)  # (B, 128+4, 1, N)

            # prediction (needed to compute loss)
            samples_pred_res1 = self.out_conv_res1(feat_res1[:, :, 0, :])  # (B, 4, N)
            samples_pred_res1 = norm_normalize(
                samples_pred_res1
            )  # (B, 4, N) - normalized

            for i in range(B):
                out_res1[i, :, rows_int[i, :], cols_int[i, :]] = samples_pred_res1[
                    i, :, :
                ]
