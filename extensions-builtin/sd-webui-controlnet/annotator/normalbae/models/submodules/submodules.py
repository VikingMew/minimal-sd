...
class UpSampleGN(nn.Module):
    def __init__(self, skip_input, output_features):
        ...
        self._net = nn.Sequential(
            Conv2d(skip_input, output_features, kernel_size=3, stride=1, padding=1),
...
def sample_points(init_normal, gt_norm_mask, sampling_ratio, beta):
    ...
    if gt_norm_mask is not None:
        gt_invalid_mask = F.interpolate(
            gt_norm_mask.float(), size=[H, W], mode="nearest"
        )
        ...
        if num_coverage <= 0:
            ...
            for i in range(B):
                ...
                coverage_list.append(
                    remaining[i, :][idx_c[:num_coverage]].view(1, -1)
                )  # 1, N-beta*N
