...
def get_act_fn(name="relu"):
    ...
    use_me = not (
        config.is_exportable() or config.is_scriptable() or config.is_no_jit()
    )
    ...
    if (
        use_jit and name in _ACT_FN_JIT
    ):  # jit scripted models should be okay for export/scripting
