...
class ConvBnAct(nn.Module):
    def __init__(
        ...
        self.conv = select_conv2d(
            in_chs, out_chs, kernel_size, stride=stride, padding=pad_type
        )
...
class EdgeResidual(nn.Module):
    ...
    def __init__(
        ...
        norm_kwargs = norm_kwargs or {}
        mid_chs = (
            make_divisible(fake_in_chs * exp_ratio)
            if fake_in_chs > 0
            else make_divisible(in_chs * exp_ratio)
        )
        self.has_residual = (in_chs == out_chs and stride == 1) and not noskip
        self.drop_connect_rate = drop_connect_rate

        # Expansion convolution
        self.conv_exp = select_conv2d(
        ...
        # Point-wise linear projection
        self.conv_pwl = select_conv2d(
            mid_chs, out_chs, pw_kernel_size, stride=stride, padding=pad_type
        )
...
def _decode_block_str(block_str):
    ...
    fake_in_chs = (
        int(options["fc"]) if "fc" in options else 0
...
def initialize_weight_default(m, n=""):
    if isinstance(m, CondConv2d):
        init_fn = get_condconv_initializer(
            partial(nn.init.kaiming_normal_, mode="fan_out", nonlinearity="relu"),
            m.num_experts,
            m.weight_shape,
        )
        init_fn(m.weight)
