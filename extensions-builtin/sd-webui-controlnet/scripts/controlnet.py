...
class Script(scripts.Script):
    ...
    def uigroup(self, tabname, is_img2img, elem_id_tabname):
        ...
        with FormRow(elem_classes="checkboxes-row", variant="compact"):
            ...
            preprocessor_preview = gr.Checkbox(
                label="Allow Preview", value=default_unit.guess_mode
            )
        ...
        with gr.Row():
            ...
            guidance_start = gr.Slider(
                ...
                value=default_unit.guidance_start,
                minimum=0.0,
                maximum=1.0,
                interactive=True,
            )
            guidance_end = gr.Slider(
                ...
                value=default_unit.guidance_end,
                minimum=0.0,
                maximum=1.0,
                interactive=True,
            )
        ...
        with advanced:
            processor_res = gr.Slider(
                label="Preprocessor resolution",
                value=default_unit.processor_res,
                minimum=64,
                maximum=2048,
                visible=False,
                interactive=False,
            ...
            threshold_a = gr.Slider(
                label="Threshold A",
                value=default_unit.threshold_a,
                minimum=64,
                maximum=1024,
                visible=False,
                interactive=False,
            ...
            threshold_b = gr.Slider(
                label="Threshold B",
                value=default_unit.threshold_b,
                minimum=64,
                maximum=1024,
                visible=False,
                interactive=False,
        ...
        if gradio_compat:
            module.change(
                build_sliders,
                inputs=[module],
                outputs=[processor_res, threshold_a, threshold_b, advanced],
            )
        ...
        def svgPreprocess(inputs):
            if inputs:
                if (
                    inputs["image"].startswith("data:image/svg+xml;base64,")
                    and svgsupport
                ):
        ...
        def run_annotator(image, module, pres, pthr_a, pthr_b):
            ...
            if pres > 64:
                result, is_image = preprocessor(
                    img, res=pres, thr_a=pthr_a, thr_b=pthr_b
                )
            else:
        ...
        preprocessor_preview.change(
            fn=shift_preview,
            inputs=[preprocessor_preview],
            outputs=[trigger_preprocessor, generated_image],
        )
        trigger_preprocessor.click(
            fn=run_annotator,
            inputs=[input_image, module, processor_res, threshold_a, threshold_b],
            outputs=[generated_image],
        )

        if is_img2img:
            send_dimen_button.click(
                fn=send_dimensions,
                inputs=[input_image],
                outputs=[self.img2img_w_slider, self.img2img_h_slider],
            )
        else:
            send_dimen_button.click(
                fn=send_dimensions,
        ...
        resize_mode = gr.Radio(
            ...
            value=default_unit.resize_mode.value,
            label="Resize Mode",
        )
        ...
        with gr.Accordion(label="Directly Draw Scribbles", open=False):
            with gr.Row():
                canvas_width = gr.Slider(
                    label="New Scribble Drawing Width",
                    minimum=256,
                    maximum=1024,
                    value=512,
                    step=64,
                ...
                canvas_height = gr.Slider(
                    label="New Scribble Drawing Height",
                    minimum=256,
                    maximum=1024,
                    value=512,
                    step=64,
                    ...
                    with gr.Tabs(elem_id=f"{elem_id_tabname}_tabs"):
                        for i in range(max_models):
                            with gr.Tab(f"Control Model - {i}"):
                                controls += (
                                    self.uigroup(
                                        f"ControlNet-{i}", is_img2img, elem_id_tabname
                                    ),
                                )
                else:
                    with gr.Column():
                        controls += (
                            self.uigroup(f"ControlNet", is_img2img, elem_id_tabname),
                        )
    ...
    def build_control_model(self, p, unet, model, lowvram):
        ...
        network_config = shared.opts.data.get(
            "control_net_model_config", global_state.default_conf
        )
        ...
        if any(
            [
                k.startswith("body.") or k == "style_embedding"
            ...
            network_config = shared.opts.data.get(
                "control_net_model_adapter_config", global_state.default_conf_adapter
            )
            if not os.path.isabs(network_config):
                network_config = os.path.join(global_state.script_dir, network_config)
        ...
        if "v11" in model_stem.lower() or "shuffle" in model_stem.lower():
            assert os.path.exists(
                override_config
            ), f"Error: The model config {override_config} is missing. ControlNet 1.1 must have configs."
    ...
    def is_ui(self, args):
        return (
            args
            and isinstance(args[0], external_code.ControlNetUnit)
            and getattr(args[0], "is_ui", False)
        )
    ...
    def process(self, p, *args):
        ...
        for idx, unit in enumerate(enabled_units):
            ...
            p_input_image = self.get_remote_call(
                p, "control_net_input_image", None, idx
            )
            ...
            is_img2img_batch_tab = (
                is_img2img
                and img2img_tab_tracker.submit_img2img_tab == "img2img_batch_tab"
            )
            if is_img2img_batch_tab and getattr(p, "image_control", None) is not None:
                ...
                if isinstance(image["image"], str):
                    ...
                    input_image = HWC3(
                        np.asarray(decode_base64_to_image(image["image"]))
                    )
                else:
            ...
            if (
                ...
                scale_x, scale_y = mask.width / float(
                    input_image[0].width
                ), mask.height / float(input_image[0].height)
                crop_region = (
                    int(x1 / scale_x),
                    int(y1 / scale_y),
                    int(x2 / scale_x),
                ...
                input_image = [
                    images.resize_image(2, x, p.width, p.height) for x in input_image
                ]
            ...
            if unit.module == "none" and "style" in unit.model:
                ...
                detected_map = np.ndarray(
                    (round(input_image.shape[0] / 4), input_image.shape[1]),
                    dtype="float32",
                    buffer=detected_map_bytes,
                )
                detected_map = torch.Tensor(detected_map).to(
            ...
            if isinstance(p, StableDiffusionProcessingTxt2Img) and p.enable_hr:
                ...
                if is_image:
                    hr_control, hr_detected_map = self.detectmap_proc(
            ...
            if is_image:
                control, detected_map = self.detectmap_proc(
                ...
                if unit.module == "clip_vision":
                    fake_detected_map = np.ndarray(
                        (detected_map.shape[0] * 4, detected_map.shape[1]),
                        dtype="uint8",
                        buffer=detected_map.detach().cpu().numpy().tobytes(),
                    )
            ...
            forward_param = ControlParams(
                ...
                is_extra_cond=getattr(model_net, "target", "")
                == "scripts.adapter.StyleAdapter",
                global_average_pooling=model_net.config.model.params.get(
        ...
        if (
            len(enabled_units) > 0
            and shared.opts.data.get("control_net_skip_img2img_processing")
            and hasattr(p, "init_images")
        ):
    ...
    def postprocess(self, p, processed, *args):
        if (
            ...
            for detect_map, module in self.detected_map:
                detectmap_dir = os.path.join(
                    shared.opts.data.get("control_net_detectedmap_dir", False), module
                )
        ...
        if (
            not no_detectmap_opt
            and hasattr(self, "detected_map")
            and self.detected_map is not None
        ):
            for detect_map, module in self.detected_map:
                if detect_map is None:
                    continue
                processed.images.extend(
                    [Image.fromarray(detect_map.clip(0, 255).astype(np.uint8))]
                )
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            global_state.default_conf,
            "Config file for Control Net models",
            section=section,
        ),
    )
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            global_state.default_conf_adapter,
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            1,
            "Model cache size (requires restart)",
            gr.Slider,
    ...
    shared.opts.add_option(
        "control_net_control_transfer",
        shared.OptionInfo(
            False,
            "Apply transfer control when loading models",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
    ...
    shared.opts.add_option(
        "control_net_skip_img2img_processing",
        shared.OptionInfo(
            False,
    ...
    shared.opts.add_option(
        "control_net_monocular_depth_optim",
        shared.OptionInfo(
            False,
            "Enable optimized monocular depth estimation",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
    ...
    shared.opts.add_option(
        "control_net_only_mid_control",
        shared.OptionInfo(
            False,
            "Only use mid-control when inference",
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            False,
            'Passing ControlNet parameters with "Send to img2img"',
            gr.Checkbox,
            {"interactive": True},
            section=section,
        ),
...
class Img2ImgTabTracker:
    ...
    def on_after_component_callback(self, component, **_kwargs):
        ...
        if type(component) is gr.Button and component.elem_id in (
            "img2img_generate",
            "txt2img_generate",
        ):
            component.click(
                fn=self.save_submit_img2img_tab,
                inputs=gr.State(component.elem_id),
                outputs=[],
            )
            return
        ...
        is_img2img_tab = (
            is_tab
            and getattr(tab, "parent", None) is not None
            and getattr(tab.parent, "elem_id", None) == "mode_img2img"
        )
        if is_img2img_tab and tab.elem_id not in self.img2img_tabs:
            tab.select(
                fn=self.set_active_img2img_tab, inputs=gr.State(tab.elem_id), outputs=[]
            )
