...
class UnetHook(nn.Module):
    def __init__(self, lowvram=False) -> None:
        ...
        self.only_mid_control = shared.opts.data.get(
    ...
    def hook(self, model):
        ...
        def cfg_based_adder(base, x, require_autocast):
            ...
            if base_h != xh or base_w != xw:
                x = th.nn.functional.interpolate(
                    x, size=(base_h, base_w), mode="nearest"
                )
        ...
        def forward(self, x, timesteps=None, context=None, **kwargs):
            ...
            for param in outer.control_params:
                ...
                # has high-res fix
                if (
                    param.hr_hint_cond is not None
                    and x.ndim == 4
                    and param.hint_cond.ndim == 3
                    and param.hr_hint_cond.ndim == 3
                ):
            ...
            if total_extra_cond is not None and outer.batch_cond_available:
                total_extra_cond = torch.repeat_interleave(
                    total_extra_cond.unsqueeze(0), context.shape[0] // 2, dim=0
                )
                if outer.is_vanilla_samplers:
                    uncond, cond = context.chunk(2)
                    cond = torch.cat([cond, total_extra_cond], dim=1)
                    uncond = torch.cat(
                        [uncond, uncond[:, -total_extra_cond.shape[1] :, :]], dim=1
                    )
            ...
            for param in outer.control_params:
                ...
                if (
                    not param.is_adapter
                    and x.shape[1] != control_model.input_blocks[0][0].in_channels
                    and x.shape[1] == 9
                ):
                ...
                if param.guess_mode or param.global_average_pooling:
                    ...
                    if param.is_adapter:
                        control = [
                            torch.concatenate(
                                [c.clone() for _ in range(query_size)], dim=0
                            )
                            for c in control
                        ]
                    uc_mask = param.generate_uc_mask(
                        query_size, dtype=x.dtype, device=x.device
                    )[:, None, None, None]
            ...
            assert timesteps is not None, ValueError(
                f"insufficient timestep: {timesteps}"
            )
            hs = []
            with th.no_grad():
                ...
                for i, module in enumerate(self.input_blocks):
                    ...
                    if ((i + 1) % 3 == 0) and len(total_adapter):
                        h = cfg_based_adder(
                            h, total_adapter.pop(0), require_inpaint_hijack
                        )
                ...
                if only_mid_control:
                    ...
                    h = th.cat(
                        [
                            h,
                            cfg_based_adder(
                                hs_input, control_input, require_inpaint_hijack
                            ),
                        ],
                        dim=1,
                    )
